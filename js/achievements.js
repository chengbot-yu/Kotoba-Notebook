/* ============================================================
   ことばノート — achievements (実績)
   Every achievement is backed by REAL data. Nothing is decorative.
   ============================================================ */
const Achv = (() => {
  /* ctx = { words, reviews, studyDays, settings, bestStreak } */
  const DEFS = [
  ...
  /* compute which achievements are newly unlocked; returns list of defs */
  function evaluate(ctx) {
    const unlocked = new Set((ctx.unlockedIds || []));
    return DEFS.filter(d => !unlocked.has(d.id) && d.check(ctx)).map(d => ({ id: d.id, unlockedAt: new Date().toISOString() }));
  }

  return { defs, def, evaluate };
})();
